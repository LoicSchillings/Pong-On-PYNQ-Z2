...
static void draw_char5x7(int x, int y, char c, int scale, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    /* Letters we need: A,C,E,I,N,O,P,R,S,T,W,G,M,V */
    ...
    static const uint8_t G_G[7] = {0x0F,0x10,0x10,0x13,0x11,0x11,0x0F};
    ...
    else if (c == 'G') glyph = G_G;
    else if (c == 'M') glyph = G_M;
    else if (c == 'V') glyph = G_V;
    else if (c == ' ') return;
