/*
 ...
 Game functions are in the top half of the file
 UDP functions are in the lower half of the file
...
static void udp_recv_perf_traffic(int sock)
{
	...
	while (1) {
		...
		if (count >= 2) {
			...
			if      (p == '1' && k == 'W') cmd = CMD_P1_UP;
			else if (p == '1' && k == 'S') cmd = CMD_P1_DOWN;
			else if (p == '2' && k == 'W') cmd = CMD_P2_UP;
			else if (p == '2' && k == 'S') cmd = CMD_P2_DOWN;
			else if (p == '1' && k == 'A') cmd = CMD_P1_READY;
			else if (p == '2' && k == 'A') cmd = CMD_P2_READY;
			else if (p == '1' && k == 'E') cmd = CMD_RESET;
			else if (p == '2' && k == 'E') cmd = CMD_RESET;
			else valid = 0;

			if (valid) {
				/* non-blocking send; drop if queue full */
				(void)xQueueSend(game_cmd_queue, &cmd, 0);
				continue; /* IMPORTANT: do NOT run iPerf parsing on this packet */
			}
		}
